...
import com.zeitheron.curseforge.api.*;
...
public class OldCheckStatsCommand implements CommandBase {
    
    public static ICurseForge curseForge;
    
    static {
        CurseForgePrefs prefs = new CurseForgePrefs();
        prefs.setCacheLifespan(new TimeHolder(1L, TimeUnit.MINUTES));
        curseForge = CurseForgeAPI.create(prefs);
    }
    ...
    public void execute(ScheduledExecutorService service, MessageCreateEvent event, MessageAuthor author, String cmd, String[] args) {
        ...
        try {
            ...
            if (args.length == 1)
                try {
                    project = curseForge.project(s).get();
                } catch (NullPointerException e) {
                
                }
            if (project == null) {
                IGame mc = curseForge.gameById("minecraft");
                ...
                List<FetchableProject> projects = curseForge.searchProjects(new CurseSearchDetails(mc, mc.categoryById("mc-mods")), s).getElements();
                if (projects.isEmpty())
                    throw new NullPointerException("Project not found!");
                projects.sort(Comparator.comparingDouble(value -> similarity(value.name().toLowerCase(Locale.ROOT), s)));
                Collections.reverse(projects);
                project = projects.get(0).fetch().get();
                if (projects.size() > 1)
                    comment = String.format("There were: %s\nBut we chose **%s**.", projects.stream().map(p -> "**" + p.name() + "**").collect(Collectors.joining(", ")), project.name());
            }
            IProjectFile latestFile = project.files().latest().fetch().get();
            EmbedBuilder updated = new EmbedBuilder();
            if (comment != null)
                updated.setDescription(comment);
            updated.setTitle(project.name());
            updated.setThumbnail(project.thumbnail());
            updated.setFooter("Requested by " + author.getDiscriminatedName(), author.getAvatar());
            updated.setColor(Color.GREEN);
            updated.addField("Project ID", project.projectId() + "");
            updated.addField("Project Created", project.createTime().toGMTString());
            updated.addField("Last Updated", project.lastUpdate().toGMTString());
            double sinceStartAverage = project.totalDownloads() / ((Instant.now().toEpochMilli() - project.createTime().toInstant().toEpochMilli()) / 1000d / 60d / 60d / 24d);
            updated.addField("Total Downloads", project.totalDownloads() + "");
            double latestAverage = latestFile.downloads() / ((Instant.now().toEpochMilli() - latestFile.uploaded().toInstant().toEpochMilli()) / 1000d / 60d / 60d / 24d);
            updated.addField("Mean Downloads (Since Start)", String.format("%.2f per day", sinceStartAverage));
            updated.addField("Mean Downloads (Latest File)", String.format("%.2f per day", latestAverage));
            updated.addField("Member(s)", project.membersList().stream().map(FetchableMember::name).collect(Collectors.joining(", ")));
            updated.setUrl(project.url());
