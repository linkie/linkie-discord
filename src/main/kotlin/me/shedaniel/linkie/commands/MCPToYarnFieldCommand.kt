...
object MCPToYarnFieldCommand : CommandBase {
    override fun execute(event: MessageCreateEvent, user: User, cmd: String, args: Array<String>, channel: MessageChannel) {
        ...
        try {
            ...
            val remappedField = mutableMapOf<String, String>()
            mcpFields.forEach { (mcpField, mcpClassParent) ->
                ...
                remappedField[mcpClassParent.intermediaryName.onlyClass() + "." + (mcpField.mappedName ?: mcpField.intermediaryName)] = (yarnClass.mappedName ?: yarnClass.intermediaryName).onlyClass() + "." + (yarnField.mappedName ?: yarnField.intermediaryName)
            }
            if (remappedField.isEmpty())
                throw NullPointerException("No results found!")
            var page = 0
            val maxPage = ceil(remappedField.size / 5.0).toInt()
            val mcpFieldsList = remappedField.keys.toList()
            message.edit { it.setEmbed { it.buildMessage(remappedField, mcpFieldsList, mappingsContainer.version, page, user, maxPage) } }.subscribe { msg ->
                        ...
                        user.id -> {
                            if (!it.emoji.asUnicodeEmoji().isPresent) {
                                ...
                                if (unicode.raw == "❌") {
                                    ...
                                    if (page > 0) {
                                        page--
                                        msg.edit { it.setEmbed { it.buildMessage(remappedField, mcpFieldsList, mappingsContainer.version, page, user, maxPage) } }.subscribe()
                                    }
                                } else if (unicode.raw == "➡") {
                                    msg.removeReaction(it.emoji, it.userId).subscribe()
                                    if (page < maxPage - 1) {
