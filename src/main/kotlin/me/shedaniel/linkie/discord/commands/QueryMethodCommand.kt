...
class QueryMethodCommand(private val namespace: Namespace?) : CommandBase {
    override fun execute(event: MessageCreateEvent, prefix: String, user: User, cmd: String, args: MutableList<String>, channel: MessageChannel) {
        ...
        message.editOrCreate(channel, event.message) { buildMessage(namespace, methods.get().second, methods.get().first, page, user, maxPage.get()) }.subscribe { msg ->
            msg.tryRemoveAllReactions().block()
            buildReactions(methods.timeToKeep) {
                if (maxPage.get() > 1) register("⬅") {
                    if (page > 0) {
                        page--
                        message.editOrCreate(channel, event.message) { buildMessage(namespace, methods.get().second, methods.get().first, page, user, maxPage.get()) }.subscribe()
                    }
                }
                registerB("❌") {
