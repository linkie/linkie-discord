...
class QueryTranslateMethodCommand(private val source: Namespace, private val target: Namespace) : CommandBase {
    override suspend fun execute(event: MessageCreateEvent, message: MessageCreator, prefix: String, user: User, cmd: String, args: MutableList<String>, channel: MessageChannel) {
        ...
        val remappedMethods = ValueKeeper(Duration.ofMinutes(2)) { build(searchTerm, source.getProvider(sourceVersion), target.getProvider(targetVersion), user, message, maxPage) }
        message.sendPages(0, maxPage.get()) { page ->
            buildMessage(remappedMethods.get(), sourceVersion, page, user, maxPage.get())
        }
    }
    ...
    private suspend fun build(
        ...
        if (!sourceProvider.cached!!) message.reply {
        ...
        else if (!targetProvider.cached!!) message.reply {
            setFooter("Requested by " + user.discriminatedName, user.avatarUrl)
            setTimestampToNow()
